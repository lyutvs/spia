    ...
    private fun renderEndpointCall(
        ...
        when (config.apiClient) {
            ...
            ApiClient.FETCH -> {
                ...
                sb.appendLine("              let __errBody: unknown = null;")
                sb.appendLine("              try { __errBody = await res.json(); } catch (_e1) {")
                sb.appendLine("                try { __errBody = await res.text(); } catch (_e2) { __errBody = null; }")
                sb.appendLine("              }")
                sb.appendLine("              throw new ApiError(res.status, __errBody, `SPIA ${endpoint.httpMethod.name} \${res.url} failed: \${res.status} \${res.statusText}`);")
                sb.appendLine("            }")
                if (promiseType != "void") {
